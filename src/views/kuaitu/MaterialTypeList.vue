...
<script setup lang="ts">
  ...
  onMounted(() => {
    getMaterialTypeList({ backTop: false })
  })

  // 搜索文章
  const searchTemplateType = () => {
    getMaterialTypeList({ backTop: true })
  }

  const getMaterialTypeList = async ({ backTop = false }) => {
    ...
    const res = await KuaituService.getMaterialTypeList(params)
    if (res.status) {
  ...
  const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate((valid, fields) => {
      if (valid) {
        if (ruleForm.id) {
          KuaituService.editMaterialType(Number(ruleForm.id), ruleForm).then((res: any) => {
            if (res.status) {
          ...
          KuaituService.addMaterialType(ruleForm).then((res: any) => {
            if (res.status) {
              ElMessage.success(res.msg)
              dialogVisible.value = false
              resetForm(ruleFormRef.value)
  ...
  const handleDel = (row: any) => {
    ElMessageBox.confirm(`确定要删除名称为：${row.name} 的类型数据吗？`, '删除确认', {
      ...
      .then(() => {
        KuaituService.delMaterialType(row.id).then((res: any) => {
          if (res.status) {
            ElMessage.success(res.msg)
            getMaterialTypeList({ backTop: true })
          }
